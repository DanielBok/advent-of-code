const PUZZLE_INPUT: &str = "1640	590	93	958	73	1263	1405	1363	737	712	1501	390	68	1554	959	79
...
pub fn solve_a() {
    let inputs = parse_input(PUZZLE_INPUT);

    let ans: i32 = inputs.iter().map(|row| {
...
pub fn solve_b() {
    ...
    let ans: i32 = inputs.iter().map(|row| {
        ...
        loop {
            if row[p2] % row[p1] == 0 {
                break row[p2] / row[p1];
            } else if row[p1] % row[p2] == 0 {
                break row[p1] / row[p2];
            } else if p2 + 1 < row.len() {
                p2 += 1
            } else {
                p1 += 1;
                p2 = p1 + 1;
