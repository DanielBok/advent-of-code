...
fn get_input(input: &str) -> (Input, Input) {
    let [wire1, wire2] = input
        ...
        .collect::<Vec<&str>>()
        [..]
        else { panic!("Could not destructure input") };

    let convert = |inp: &str| -> Input {
        inp
            .split(",")
            .map(|x| {
                ...
                let steps = chars.as_str().parse::<i32>().unwrap_or_else(|_| {
                    panic!("Could not parse {x}");
                });
    ...
    (convert(wire1), convert(wire2))
}

pub fn solve_a() {
    let (wire1, wire2) = get_input(PUZZLE_INPUT);
    let closest = closest_intersection_distance(&wire1, &wire2);

    assert_eq!(closest, 5319);
    println!("Solution A: {}", closest);
}

fn closest_intersection_distance(wire1: &Input, wire2: &Input) -> i32 {
    let map1 = get_travelled_locations(&wire1);
    let map2 = get_travelled_locations(&wire2);

    let mut distance = -1;
    for key in map1.keys() {
        if map2.contains_key(key) {
            let current_distance = key.0.abs() + key.1.abs();
            if distance == -1 || distance > current_distance {
                distance = current_distance
            }
        }
    }

    assert!(distance > 0, "Could not find any intersections");
    distance
}

fn minimum_steps_to_intersection(wire1: &Input, wire2: &Input) -> i32 {
    let map1 = get_travelled_locations(&wire1);
    let map2 = get_travelled_locations(&wire2);

    let mut min_steps = -1;
    for (key, step1) in map1 {
        if let Some(step2) = map2.get(&key) {
            let total_steps = step1 + step2;
            if min_steps == -1 || total_steps < min_steps {
...
fn get_travelled_locations(wire: &Input) -> HashMap<(i32, i32), i32> {
    let mut x = 0;
    let mut y = 0;
    let mut time = 0;

    let mut travelled: HashMap<(i32, i32), i32> = HashMap::new();
    for (dir, n_steps) in wire {
        for _ in 0..*n_steps {
            time += 1;
            match dir {
                Direction::UP => { y += 1; }
            ...
            let key = (x, y);
            if !travelled.contains_key(&key) {
                travelled.insert(key, time);
            }
        }
    }
    travelled
...
pub fn solve_b() {
    let (wire1, wire2) = get_input(PUZZLE_INPUT);
    let min_steps = minimum_steps_to_intersection(&wire1, &wire2);

    assert_eq!(min_steps, 122514);
    println!("Solution B: {}", min_steps);
}


#[cfg(test)]
mod tests {
    use crate::d3::{closest_intersection_distance, get_input, minimum_steps_to_intersection};
...
U98,R91,D20,R16,D67,R40,U7,R15,U6,R7", 135)
            ...
            let (wire1, wire2) = get_input(inp);
            assert_eq!(closest_intersection_distance(&wire1, &wire2), exp)
        }
    }

    #[test]
    fn test_minimum_steps_to_intersection() {
        for (inp, exp) in [
            ("R75,D30,R83,U83,L12,D49,R71,U7,L72
...
U98,R91,D20,R16,D67,R40,U7,R15,U6,R7", 410)
            ...
            let (wire1, wire2) = get_input(inp);
            assert_eq!(minimum_steps_to_intersection(&wire1, &wire2), exp)
        }
    }
}
